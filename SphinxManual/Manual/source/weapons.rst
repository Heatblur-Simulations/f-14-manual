...
This is done on the RIO's armament panel with the MSL OPTIONS switch by setting it to SP PD (sparrow pulse-doppler).
...
When using boresight or ACM without a track the missile will launch along the ADL locking onto the first target seen and while launching at a non pulse doppler radar track the seeker head will be prepositioned onto that track.
