...
The upper position, **BYPASS**, is momentary and holding the switch to that position allows all signals in that band through regardless of their parameters, normally only those matching threats are passed through.
Holding a band to **BYPASS** removes the other two band from the display completely though which is why the switch is automatically returned to **NORM** when released.
The **NORM** position allows for normal operation of that band.
The lower position, **DEFEAT**, is locked out and the switch need to be lifted to allow movement to this position. The **DEFEAT** position completely disables the selected band.
...
The **ML** position allows override if a **ML** condition occurs.
The **ML/AI** position allows override if a **ML** condition occurs or an **AI** (Airborne Intercept, i.e. fighter radar) is detected.
The **OFF** position disables override.
