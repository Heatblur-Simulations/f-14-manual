...
The approach indexer provides visual indication of aircraft angle of attack
relative to on-speed AOA during landing.
...
When the HOOK BYPASS switch is set to CARRIER and the landing gear is down, the
indexer lights will flash if the arresting hook is not extended.

These indications are repeated on the nosewheel strut approach lights, allowing
the landing signal officer (LSO) to observe aircraft AOA during carrier
recoveries.

## HUD Caution Lights

![HUD left side indicators](../../../img/bu/left_windshield_frame_caution_lights_left_windshield_frame.jpg)
