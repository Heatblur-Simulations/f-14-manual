...
The heads-up display (HUD) projects flight and weapon delivery information onto
the forward portion of the canopy/windscreen.

Night mode is selected using the control on the right side of the VDI.

Two engine stall warning lights, L STALL and R STALL, are mounted on the left
and right sides of the HUD, respectively. Each light indicates an engine stall
condition in the corresponding engine.
...
delivery. It can be removed/hidden by clicking on it. Realistically, some pilots
carried a wrench and removed it for improved visibility during carrier landings.
...
The ACM (Air Combat Maneuver) switch/cover (<num>1</num>) enables ACM mode when
lifted and provides access to the ACM JETT button.

### ACM Jettison Button

The ACM JETT button (<num>2</num>) is located under the ACM cover and commands
...
The SEAM LOCK light (<num>3</num>) illuminates to indicate that Sidewinder
...
The COLLISION light (<num>4</num>) indicates collision steering has been
selected during AWG-9 STT operation.

### Hot Trigger Light

The HOT TRIG light (<num>5</num>) illuminates when HOT TRIGGER conditions are
satisfied. When illuminated, pulling the trigger will release a weapon.

### Gun Rate Pushbutton

Toggle pushbutton with light indication (<num>6</num>) selecting the M61 gun
firing rate.

- HIGH - Selects 6,000 rounds per minute. Normally used for air-to-air
  operation.
- LOW - Selects 4,000 rounds per minute. Normally used for air-to-ground
  operation.
...
Toggle pushbutton with light indication (<num>7</num>) providing manual control
...
Toggle pushbutton with light indication (<num>8</num>) commanding the WCS to
...
Toggle pushbutton with light indication (<num>9</num>) selecting missile launch
mode.
...
The MASTER ARM switch (<num>10</num>) enables weapons release and selective and
auxiliary jettison.
...
- CHECKERBOARD - Weapon selected and ready for launch. While on the ground, this
  indicates fuselage rails are up and locked and that loaded weapons are armed.

### Master Caution Light and Button

The MASTER CAUTION light and reset button (<num>12</num>) flashes to indicate a
...
The L FIRE and R FIRE lights (<num>13</num>) illuminate when fire is detected in
the respective engine.

### Turn-and-Slip Indicator

The turn-and-slip indicator (<num>14</num>) displays rate of turn about the
...
The vertical display indicator (VDI) complements the HUD by displaying flight
...
The FILTER handle (<num>4</num>) inserts the night filter for HUD night
operation when pulled.
...
VDI-mounted caution lights (<num>7</num>) provide data link warning and caution
indications.
...
With flaps up, this button commands the ALE-39 to dispense chaff or flares per
RIO settings.
