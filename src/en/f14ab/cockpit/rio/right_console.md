...
The inventory counters (<num>1</num>) indicate remaining countermeasure
quantities.

Counters are manually set by pushing in and rotating the knob beneath each
counter.

### Power and Mode Switch

The PWR/MODE switch (<num>2</num>) applies power and selects AN/ALE-39 operating
mode.

- AUTO(CHAFF)/MAN - Enables manual dispensing and automatic chaff dispensing via
  ECM threat detection.
...
The FLARE MODE switch (<num>4</num>) selects flare dispensing logic and also
controls what the pilot’s DLC button dispenses while airborne.

- MULT - Each flare command dispenses one flare from each launcher subsection
  loaded with flares.
- NORM - Each flare command dispenses one flare total.
- PILOT - Pilot DLC button dispenses flares. Other positions cause the pilot DLC
  button to dispense chaff.

### Countermeasure Release Switches

The countermeasure release switches (<num>5</num>) are spring-loaded to center
and command countermeasure release.

- PRGM - Program release. Switch must be lifted to enter PRGM; momentary
...
Programming panel used to enter countermeasure programs and inventory into the
AN/ALE-39.
...
The LOAD control thumbwheels (<num>4</num>) input what is loaded in each
launcher subsection.
...
The RESET switch (<num>5</num>) loads programmed settings into the AN/ALE-39 and
resets sequencer switches to their start positions.
...
The CONSOLE thumbwheel (<num>3</num>) controls console lighting and red flood
lighting.
...
The RED FLOOD switch (<num>4</num>) controls red instrument and console flood
lighting.
...
The DATA/ADF switch (<num>1</num>) controls display of navigation data and ADF
symbology on the HSD and ECMD.
...
Panel containing IFF antenna selection, BIT controls, and ground cooling
control.
...
The canopy air diffuser lever (<num>1</num>) controls distribution of
