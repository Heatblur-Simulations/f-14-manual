...
Panneau de commande pour le récepteur d'avertissement radar ALR-67.
...
Indicateur de données numériques utilisé pour afficher les commandes reçues via la liaison de
données.
...
| ORBIT | Maintenez une orbite à la position actuelle en conservant une endurance maximale. |
...
| WAVE OFF | Remise des gaz, désengagement automatique AFCS. |
...
| 10 SEC | Indique 10 secondes avant la prochaine action ou le prochain waypoint. En ACL indique que le mouvement du navire est pris en compte pour l'ACL. |
...
| TILT | Indique qu'aucun message de liaison de données n'a été reçu au cours des 10 dernières secondes. En ACL indique qu'aucun message n'a été envoyé au cours des 2 dernières secondes, désengagera AFCS. |
...
> 💡 La majorité des voyants DDI dépendent des messages de réponse de la liaison de données
> et ne sont pas actuellement modélisés dans DCS.

## Panneau de configuration DECM

![decm](../../../img/cockpit_detail_rio_decm.jpg)

Panneau de commande pour le brouilleur AN/ALQ-126 DECM.
...
![liaison de données](../../../img/cockpit_detail_rio_datalink.jpg)

Panneau de commande pour le fonctionnement de la liaison de données.
...
![Antenne de liaison de données](../../../img/cockpit_detail_rio_datalinkantenna.jpg)

Panneau contrôlant l’alignement de la liaison de données, la réponse et la sélection de l’antenne.
...
Contrôle la distribution des contre-mesures et les modes de fonctionnement de l'AN/ALE-39.

### Compteurs d'inventaire

Les compteurs d'inventaire (<num>1</num>) indiquent les quantités de
contre-mesures restantes.

Les compteurs sont réglés manuellement en poussant et en tournant le bouton sous chaque
comptoir.

### Commutateur d'alimentation et de mode

Le commutateur PWR/MODE (<num>2</num>) applique l'alimentation et sélectionne le
mode de fonctionnement de l'AN/ALE-39.

- AUTO(CHAFF)/MAN - Permet la distribution manuelle et la distribution automatique des paillettes
  via
Détection des menaces ECM.
- MAN - Distribution manuelle uniquement.
- OFF - Système éteint.

### Commutateur de fusées éclairantes Salvo

Le commutateur SALVO FLARES (<num>3</num>) permet un tir rapide de toutes les fusées éclairantes.

L'interrupteur doit être levé pour passer à ON.

### Commutateur de mode de fusée éclairante

Le commutateur FLARE MODE (<num>4</num>) sélectionne la logique de distribution des fusées
éclairantes et également
contrôle ce que le bouton DLC du pilote distribue en vol.

- MULT - Chaque commande de fusée distribue une fusée éclairante à partir de chaque sous-section du
  lanceur
chargé de fusées éclairantes.
- NORM - Chaque commande de fusée distribue une fusée au total.
- PILOT - Le bouton Pilot DLC distribue des fusées éclairantes. D'autres positions provoquent le DLC
  pilote
bouton pour distribuer les paillettes.

### Commutateurs de déclenchement de contre-mesures

Les interrupteurs de déverrouillage des contre-mesures (<num>5</num>) sont à ressort au centre
et commander le déclenchement des contre-mesures.

- PRGM - Sortie du programme. L'interrupteur doit être levé pour accéder au PRGM ; momentané
l’actionnement déclenche le déclenchement programmé.
- STBY – Aucun largage.
- SGL - Largage unique.
...
La première molette définit le mode, et les quatre dernières molettes définissent le code.
...
## Programmeur AN/ALE-39

![ale39programmeur](../../../img/cockpit_detail_rio_ale39programmer.jpg)

Panneau de programmation utilisé pour saisir les programmes de contre-mesures et l'inventaire dans
le
AN/ALE-39.

### Molettes du programme Jammer

Les molettes JAMR (<num>1</num>) définissent les paramètres du programme du brouilleur.

- QTY - Quantité de brouilleurs libérés par programme.
- INTV - Intervalle en secondes entre les libérations du brouilleur.

### Molette du programme Chaff

Les molettes CHAFF (<num>2</num>) définissent les paramètres du programme de paillettes.

- B-QTY - Quantité d'éclatement par salve ; peut être réglé sur C (continu) ou R (aléatoire).
- B-INTV - Intervalle de rafale ; peut être réglé sur R (aléatoire).
- S-QTY - Nombre de salves dans le programme.
- S-INTV - Intervalle entre les salves.

### Molettes du programme Flare

Les molettes FLARE (<num>3</num>) définissent les paramètres du programme Flare.

- QTY - Quantité de fusées éclairantes libérées par programme.
- INTV - Intervalle entre les déclenchements de fusées éclairantes.

### Molette de contrôle de charge

Les molettes de commande LOAD (<num>4</num>) saisissent ce qui est chargé dans chaque
sous-section du lanceur.

- L10 - Charge de sous-section 10 gauche.
- L20 - Charge de sous-section gauche 20.
- R10 - Charge de sous-section 10 droite.
- R20 - Charge de sous-section droite 20.

### Réinitialiser le commutateur

Le commutateur RESET (<num>5</num>) charge les paramètres programmés dans l'AN/ALE-39 et
réinitialise les commutateurs du séquenceur à leurs positions de départ.

## Panneau de commande d'éclairage intérieur

![voyant](../../../img/cockpit_detail_rio_light.jpg)

Panneau de contrôle de l'éclairage du cockpit RIO.
...
La molette CONSOLE (<num>3</num>) contrôle l'éclairage de la console et l'inondation rouge
éclairage.

- 0 - Console et inondation rouge éteintes
- 1–14 - Augmentation de la luminosité de la console

### Interrupteur d'inondation rouge

L'interrupteur RED FLOOD (<num>4</num>) contrôle l'inondation rouge des instruments et de la
console.
éclairage.

- DIM - Dim inondation de console rouge
- MED - Console rouge médium Flood
- BRT – Feux d'instruments et de console rouge vif

## Commutateur Données/ADF

![données](../../../img/cockpit_detail_rio_data.jpg)

Le commutateur DATA/ADF (<num>1</num>) contrôle l'affichage des données de navigation et de l'ADF.
symbologie sur le HSD et l'ECMD.

- BOTH - Affiche le bug ADF et le bloc de données de navigation.
- DATA - Affiche uniquement le bloc de données de navigation.
- OFF - Désactive les deux.

## Panneau de contrôle du transpondeur IFF

![transpondeur](../../../img/cockpit_detail_rio_transponder.jpg)

Panneau de commande pour le transpondeur IFF AN/APX-72.
...
- LOW - Réponses à faible sensibilité ; ne répond qu'aux interrogateurs puissants à proximité.
...
- OUT - IDENT désactivé.
- MIC - Transfère le contrôle IDENT au membre d'équipage UHF PTT ; Les réponses IDENT se produisent
  lorsque
PTT est saisi.
...
Panneau contenant la sélection d'antenne IFF, les commandes BIT et le refroidissement au sol
contrôle.
...
## Panneau de test de dérivation de compression moyenne (F-14A uniquement)

![mcb](../../../img/cockpit_detail_rio_mcb.jpg)

Panneau utilisé pour vérifier la fonctionnalité du circuit Mid Compression Bypass (MCB).
...
## Panneau de test du système d'alimentation électrique

![test du système électrique](../../../img/cockpit_detail_rio_powersystest.jpg)

Panneau de test du personnel au sol pour les systèmes électriques des avions.

> 💡 Non fonctionnel en DCS.

## Désembuage de la verrière/levier d'air de la cabine

![désembuage de la verrière](../../../img/cockpit_detail_rio_canopydefog.jpg)

Le levier du diffuseur d'air de la verrière (<num>1</num>) contrôle la distribution de
air conditionné de la cabine.

- CABIN AIR - Position normale. Dirige environ 70 % du flux d'air à travers
